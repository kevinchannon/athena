...
2. **`docs`** (fallback, if present) — Author-written docstring
3. **`sig`** (fallback, guaranteed to be present) — Structural description from AST (signature for functions, type and scope for variables, members variables and methods for classes)
...
{"file": "src/auth/session.ts", "lines": "88-105"}
